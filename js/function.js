...
$(function() {
  ...
  $btnGnb.on('click', function(){
		...
		// 만약 clse를 가지고 있으면 >> 메뉴 활성화 상태(X)로 메뉴 닫으려면(종료) >>
...
$(function(){
	...
	$(window).on("scroll", function(){
		if($(this).scrollTop() > $(".workstyle").offset().top-$(this).height()){
			$('.workstyle').find('.before').addClass('animate');
			$('.workstyle').find('.after').addClass('animate2');
		}else{
			$('.workstyle').find('.before').removeClass('animate');
			$('.workstyle').find('.after').removeClass('animate2');
		}
	});
...
$(function(){
  ...
  // 함수는 반복되는 코드를 만들어 놓고 사용하면 코드의 재활용 측면에서 유용하게 사용 가능
